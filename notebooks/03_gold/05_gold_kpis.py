...
# MAGIC %md
# MAGIC # View: workspace.gold.vw_kpi_business
# MAGIC ## Objetivo:
...
# MAGIC |--------|-------------|
# MAGIC | `workspace.gold.fact_pedido` | Driver (granularidade da view) |
# MAGIC | `workspace.gold.fact_entrega` | Agregada por order_id para `flag_atrasado` e tempo médio |
# MAGIC | `workspace.gold.fact_ocorrencia` | Agregada por order_id para `flag_com_ocorrencia` |
# MAGIC | `workspace.gold.dim_*` | Cliente, produto, canal, região, vendedor, data |
# MAGIC
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE VIEW workspace.gold.vw_kpi_business AS
# MAGIC SELECT
...
# MAGIC
# MAGIC -- Atributos de cliente (selecionar conforme schema real do workspace.silver.clientes)
...
# MAGIC
# MAGIC FROM workspace.gold.fact_pedido p
# MAGIC LEFT JOIN workspace.gold.dim_data d ON p.data_id = d.data_id
# MAGIC LEFT JOIN workspace.gold.dim_cliente c ON p.customer_code = c.customer_code
# MAGIC LEFT JOIN workspace.gold.dim_vendedor v ON p.seller_id = v.seller_id
# MAGIC LEFT JOIN workspace.gold.dim_canal ca ON v.canal_id = ca.canal_id
# MAGIC LEFT JOIN workspace.gold.dim_regiao r ON v.regional_code = r.regional_code
# MAGIC LEFT JOIN (
# MAGIC SELECT
# MAGIC order_id,
# MAGIC COUNT(*) AS qtd_itens,
# MAGIC COUNT(DISTINCT product_code) AS qtd_skus_distintos
# MAGIC FROM workspace.gold.fact_item
...
# MAGIC FROM workspace.gold.fact_entrega
# MAGIC GROUP BY order_id
...
# MAGIC FROM workspace.gold.fact_ocorrencia
# MAGIC GROUP BY order_id
...
print("[OK] workspace.gold.vw_kpi_business criada.")
spark.sql("SELECT COUNT(*) AS total_pedidos FROM workspace.gold.vw_kpi_business").show()
spark.sql("SELECT * FROM workspace.gold.vw_kpi_business LIMIT 5").show(truncate=False)
...
# MAGIC FROM workspace.gold.vw_kpi_business
